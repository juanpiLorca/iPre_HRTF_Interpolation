...
def encoder_layers(inputs, 
    ...
    x = tfkl.ReLU()(x)
    last_conv = tfkl.Conv1D(filters=int(filters/2), kernel_size=kernel_size, 
                            strides=1, padding="same", 
                            kernel_regularizer="l2")(x)
    x = tfkl.ReLU()(last_conv)
...
# Decoder: ---------------------------------------------------------------------------------------------------------
# Initialization weights for convolutions in order to use sine activation functions: 
def random_init(ch_in): 
    random = tf.random_uniform_initializer(minval=-np.sqrt(6.0/ch_in*kernel_size), 
                                           maxval=np.sqrt(6.0/ch_in*kernel_size))
    return random
